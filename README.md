...
If you defined a block called x using the `block` column, you can access it with
...
Finally, reprojuicer loads up your environment with a transformed dataset `old_data_set_name.r` and also a data.frame for each block of variables you created, for easy access. 
...
You have imported questionnaire data from an online survey tool and now you need to tidy lots of ...
...
Just type a name for your recode, say `gender`, but it could be anything, in the recode column in the row for the gender variable in your reprojuicer spreadsheet. Run `reprojuicer("mydata")` as usual. Depending on your spreadsheet software, you might need to refresh the spreadsheet (in Libreoffice / Openoffice this is File/Reload). You should see a new tab called gender with the levels of the variable (0 and 1) in the rows of two columns, output and input. So to recode, you just change the values in the output column to whatever you want, say male and female.

Run `reprojuicer("mydata")` one more time and inspect mydata.r$gender. 

Additional features: 

- you can easily recode many variables at the same time by just typing the same name in the recode column for the variables in question
...
You can put e.g. 4:8 or lo:3 or 4:hi or else in the column labelled “Input”.

## Using the spreadsheet

You should be able to style it, i.e. change colours and column widths, and they should be retained.
Remember to save your prop file before running reprojuicer! reprojuicer should then close and reopen your file, possibly changed by the script.
...
You should be able to add and delete columns in your raw data file and they get automagically updated in the props file after you run reprojuicer(“nameOfDatase”)
...
## Defining blocks and the Findr function

You can add any columns in the spreadsheet you want e.g. newcol, and fill it with anything you want, mainly to make it easy to select different blocks. 
